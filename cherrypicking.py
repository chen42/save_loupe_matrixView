...
# save matrix view images for particular chr and image
whichchr=6
imgs=[155, 175, 219]
timeBetweenImages=20 # wait long enough for the difficult images to load
...
def saveMVperChr(chr, imgs, timeBetweenImages):
    ...
    for cnt in imgs:
        ...
        if not os.path.isfile(filename):
            ...
            #print (url)
            browser.get(url)
            time.sleep(timeBetweenImages) # set it to allow the image to load into the browser
...
for i in [16]:
    ...
    saveMVperChr(whichchr, imgs, timeBetweenImages)
